...
<script setup>
			...
			if (response.data.code === 200) {
				...
				const materialPromises = materialItems.map(async (item) => {
					try {
						...
						// 调用getFile接口获取视频数据
						const fileResponse = await axios.get(`/getFile`, {
							params: {
								filename: filename
							},
							responseType: 'blob'
	...
	const fetchAccounts = async (forceRefresh = false) => {
		...
		try {
			const response = await axios.get(`/getValidAccounts`);
    ...
    // console.log('账号信息:', publishCards.value);
		...
		try {
			...
			// 发送发布请求
			// 发送发布请求
			const publishResponse = await axios.post('/postVideo', postData);
	...
	const deleteMaterial = async (material) => {
		...
		if (confirm(`确定要删除素材 ${material.filename || material.file_name} 吗？`)) {
			try {
				// 调用删除API
				const response = await axios.get(`/deleteFile?id=${material.id}`);
