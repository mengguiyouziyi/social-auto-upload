            ...
            <div class="quality-analysis">
              <div class="analysis-tabs">
                <el-tabs v-model="activeAnalysisTab">
                  ...
                  <el-tab-pane label="平台分析" name="platforms">
                    <div class="platform-analysis">
                      <el-row :gutter="20">
                        <el-col :span="12">
                          <div class="chart-container">
                            <h4>平台表现对比</h4>
                            <div class="platform-performance-chart" ref="platformPerformanceChart"></div>
                          </div>
                        </el-col>
                        <el-col :span="12">
                          <div class="chart-container">
                            <h4>内容类型分布</h4>
                            <div class="content-type-chart" ref="contentTypeChart"></div>
                          </div>
                        </el-col>
                      </el-row>
                      <el-row :gutter="20" style="margin-top: 20px;">
                        <el-col :span="24">
                          <div class="chart-container">
                            <h4>时间趋势分析</h4>
                            <div class="time-trend-chart" ref="timeTrendChart"></div>
                          </div>
                        </el-col>
                      </el-row>
                    </div>
                  ...
                  <el-tab-pane label="用户行为" name="user-behavior">
                    <div class="user-behavior-analysis">
                      <el-row :gutter="20">
                        <el-col :span="12">
                          <div class="chart-container">
                            <h4>用户活跃热力图</h4>
                            <div class="user-heatmap-chart" ref="userHeatmapChart"></div>
                          </div>
                        </el-col>
                        <el-col :span="12">
                          <div class="chart-container">
                            <h4>互动类型分布</h4>
                            <div class="interaction-type-chart" ref="interactionTypeChart"></div>
                          </div>
                        </el-col>
                      </el-row>
                    </div>
                      ...
                      <div class="competitor-charts">
                        <el-row :gutter="20">
                          <el-col :span="12">
                            <div class="chart-container">
                              <h4>粉丝数量对比</h4>
                              <div class="competitor-fans-chart" ref="competitorFansChart"></div>
                            </div>
                          </el-col>
                          <el-col :span="12">
                            <div class="chart-container">
                              <h4>互动率对比</h4>
                              <div class="competitor-engagement-chart" ref="competitorEngagementChart"></div>
                            </div>
                          </el-col>
                        </el-row>
                      </div>
...
const platformData = ref([
  { name: '抖音', value: 85, growth: 12.5, color: '#FF6B6B' },
  { name: '小红书', value: 78, growth: 8.3, color: '#4ECDC4' },
  { name: 'B站', value: 72, growth: 15.2, color: '#45B7D1' },
  { name: '快手', value: 68, growth: 6.7, color: '#96CEB4' },
  { name: '视频号', value: 45, growth: 22.1, color: '#FFEAA7' }
])

const contentTypeData = ref([
  { name: '短视频', value: 45, color: '#FF6B6B' },
  { name: '图文', value: 30, color: '#4ECDC4' },
  { name: '直播', value: 15, color: '#45B7D1' },
  { name: '纯文案', value: 10, color: '#96CEB4' }
])

const timeTrendData = ref([
  { date: '2024-01', engagement: 65, reach: 1200 },
  { date: '2024-02', engagement: 72, reach: 1350 },
  { date: '2024-03', engagement: 78, reach: 1500 },
  { date: '2024-04', engagement: 85, reach: 1680 },
  { date: '2024-05', engagement: 82, reach: 1600 },
  { date: '2024-06', engagement: 88, reach: 1750 }
])

// 用户行为数据
const userHeatmapData = ref([])
const interactionData = ref([
  { name: '点赞', value: 45, color: '#FF6B6B' },
  { name: '评论', value: 30, color: '#4ECDC4' },
  { name: '分享', value: 15, color: '#45B7D1' },
  { name: '收藏', value: 10, color: '#96CEB4' }
])
...
const initPlatformCharts = () => {
  // 平台表现对比图表
  const platformChartDom = document.querySelector('.platform-performance-chart')
  if (platformChartDom) {
    const platformChart = echarts.init(platformChartDom)
    const option = {
      tooltip: {
        trigger: 'axis',
        axisPointer: {
      ...
      xAxis: {
        type: 'category',
        data: platformData.value.map(item => item.name)
      },
      yAxis: {
        type: 'value',
        name: '表现分数'
  ...
  // 内容类型分布图表
  const contentTypeChartDom = document.querySelector('.content-type-chart')
  if (contentTypeChartDom) {
    const contentTypeChart = echarts.init(contentTypeChartDom)
    const option = {
      tooltip: {
        trigger: 'item'
      },
  ...
  // 时间趋势分析图表
  const timeTrendChartDom = document.querySelector('.time-trend-chart')
  if (timeTrendChartDom) {
    const timeTrendChart = echarts.init(timeTrendChartDom)
    const option = {
      tooltip: {
        trigger: 'axis'
      },
      ...
      xAxis: {
        type: 'category',
        data: timeTrendData.value.map(item => item.date)
      },
      yAxis: [
        {
          type: 'value',
      ...
      series: [
        {
          name: '互动率',
          type: 'line',
          data: timeTrendData.value.map(item => item.engagement),
          smooth: true,
        ...
        },
        {
          ...
          type: 'line',
          yAxisIndex: 1,
          data: timeTrendData.value.map(item => item.reach),
          smooth: true,
...
const initUserBehaviorCharts = () => {
  // 用户活跃热力图
  const userHeatmapChartDom = document.querySelector('.user-heatmap-chart')
  if (userHeatmapChartDom) {
    const userHeatmapChart = echarts.init(userHeatmapChartDom)
    // 生成模拟热力图数据
    const heatmapData = []
    ...
    userHeatmapChart.setOption(option)
  }

  // 互动类型分布图表
  const interactionChartDom = document.querySelector('.interaction-type-chart')
  if (interactionChartDom) {
    const interactionChart = echarts.init(interactionChartDom)
    const option = {
      tooltip: {
        trigger: 'item'
      },
...
const initCompetitorCharts = () => {
  // 粉丝数量对比图表
  const competitorFansChartDom = document.querySelector('.competitor-fans-chart')
  if (competitorFansChartDom) {
    const competitorFansChart = echarts.init(competitorFansChartDom)
    const option = {
      tooltip: {
        trigger: 'axis',
        axisPointer: {
    ...
    competitorFansChart.setOption(option)
  }

  // 互动率对比图表
  const competitorEngagementChartDom = document.querySelector('.competitor-engagement-chart')
  if (competitorEngagementChartDom) {
    const competitorEngagementChart = echarts.init(competitorEngagementChartDom)
    const option = {
      tooltip: {
        trigger: 'axis'
      },
      ...
      series: [{
        type: 'radar',
        data: competitors.value.map((competitor, index) => ({
          value: [
          ...
          areaStyle: {
            color: index === 0 ? 'rgba(64, 158, 255, 0.2)' : 'rgba(103, 194, 58, 0.2)'
          },
          lineStyle: {
    ...
    competitorEngagementChart.setOption(option)
  }
}
...
.competitor-analysis {
  .chart-container {
    ...
    .competitor-fans-chart,
    .competitor-engagement-chart {
      height: 300px;
    }
  }
}
