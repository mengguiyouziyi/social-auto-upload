  ...
  <div class="user-management">
    ...
    <div class="management-header">
      ...
      <div class="header-actions">
        ...
        <el-dropdown @command="showAdvancedPermissionDialog">
          <el-button type="warning">
            <el-icon><Setting /></el-icon>高级权限
          </el-button>
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="inheritance">权限继承</el-dropdown-item>
              <el-dropdown-item command="delegation">权限委托</el-dropdown-item>
              <el-dropdown-item command="data_permissions">数据权限</el-dropdown-item>
              <el-dropdown-item command="audit">审计配置</el-dropdown-item>
              <el-dropdown-item command="security" divided>安全策略</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      </div>
            ...
            <template #header>
              <div class="card-header">
                ...
                <div class="header-actions">
                  ...
                  <el-button @click="selectAllUsers" size="small">
            ...
            <div class="users-table">
              ...
              <div v-if="selectedUsers.length > 0" class="batch-actions">
                ...
                <div class="batch-buttons">
                  ...
                  <el-dropdown @command="batchAssignRole">
                    <el-button size="small">
                    ...
                    <template #dropdown>
                      <el-dropdown-menu>
                        ...
                        <el-dropdown-item command="guest">访客</el-dropdown-item>
                      </el-dropdown-menu>
                    </template>
                  </el-dropdown>
                  <el-button size="small" @click="batchDisableUsers" type="warning">
        ...
        <!-- 安全设置 -->
              ...
              <div class="setting-item">
                <div class="setting-info">
                  ...
                  <div class="policy-config">
                    <el-checkbox v-model="securitySettings.passwordPolicy.minLength" :disabled="!securitySettings.passwordPolicy.enabled">
                      最小长度 {{ securitySettings.passwordPolicy.minLengthValue }} 位
                    </el-checkbox>
                    <el-checkbox v-model="securitySettings.passwordPolicy.requireUppercase" :disabled="!securitySettings.passwordPolicy.enabled">
                      必须包含大写字母
                    </el-checkbox>
                    <el-checkbox v-model="securitySettings.passwordPolicy.requireNumbers" :disabled="!securitySettings.passwordPolicy.enabled">
                      必须包含数字
                    </el-checkbox>
                    <el-checkbox v-model="securitySettings.passwordPolicy.requireSpecialChars" :disabled="!securitySettings.passwordPolicy.enabled">
                      必须包含特殊字符
                    </el-checkbox>
                  </div>
                </div>
                <div class="setting-actions">
                  <el-switch
                    v-model="securitySettings.passwordPolicy.enabled"
                    @change="togglePasswordPolicy"
                  />
                </div>
              </div>
              <div class="setting-item">
                <div class="setting-info">
                  ...
                  <div class="policy-config">
                    <div class="config-item">
                      <span>失败次数限制:</span>
                      <el-input-number
                        v-model="securitySettings.loginSecurity.maxAttempts"
                        :min="1"
                        :max="10"
                        size="small"
                        :disabled="!securitySettings.loginSecurity.enabled"
                      />
                    </div>
                    <div class="config-item">
                      <span>锁定时间(分钟):</span>
                      <el-input-number
                        v-model="securitySettings.loginSecurity.lockTime"
                        :min="1"
                        :max="60"
                        size="small"
                        :disabled="!securitySettings.loginSecurity.enabled"
                    ...
                    <el-checkbox v-model="securitySettings.loginSecurity.requireCaptcha" :disabled="!securitySettings.loginSecurity.enabled">
                      登录需要验证码
                    </el-checkbox>
                  </div>
                </div>
                <div class="setting-actions">
                  <el-switch
                    v-model="securitySettings.loginSecurity.enabled"
                    @change="toggleLoginSecurity"
                  />
                </div>
              </div>
              <div class="setting-item">
                <div class="setting-info">
                  ...
                  <div class="policy-config">
                    <div class="config-item">
                      <span>会话超时(分钟):</span>
                      <el-input-number
                        v-model="securitySettings.sessionManagement.timeout"
                        :min="5"
                        :max="480"
                        size="small"
                        :disabled="!securitySettings.sessionManagement.enabled"
                      />
                    </div>
                    <div class="config-item">
                      <span>最大并发数:</span>
                      <el-input-number
                        v-model="securitySettings.sessionManagement.maxConcurrent"
                        :min="1"
                        :max="10"
                        size="small"
                        :disabled="!securitySettings.sessionManagement.enabled"
                ...
                <div class="setting-actions">
                  <el-switch
                    v-model="securitySettings.sessionManagement.enabled"
                    @change="toggleSessionManagement"
                  />
                </div>
              </div>
              <div class="setting-item">
                <div class="setting-info">
                  ...
                  <div class="policy-config">
                    <div class="config-item">
                      <span>IP白名单:</span>
                      <el-input
                        v-model="securitySettings.accessControl.ipWhitelist"
                        type="textarea"
                        :rows="2"
                        placeholder="输入IP地址，用逗号分隔"
                        :disabled="!securitySettings.accessControl.enabled"
                      />
                    </div>
                    <el-checkbox v-model="securitySettings.accessControl.timeRestriction" :disabled="!securitySettings.accessControl.enabled">
                      启用时间限制
                    </el-checkbox>
                    <div v-if="securitySettings.accessControl.timeRestriction && securitySettings.accessControl.enabled" class="config-item">
                      <span>允许访问时间:</span>
                      <el-time-picker
                        v-model="securitySettings.accessControl.startTime"
                        placeholder="开始时间"
                        format="HH:mm"
                        value-format="HH:mm"
                        size="small"
                      ...
                      <span>-</span>
                      <el-time-picker
                        v-model="securitySettings.accessControl.endTime"
                        placeholder="结束时间"
                        format="HH:mm"
                        value-format="HH:mm"
                        size="small"
                ...
                <div class="setting-actions">
                  <el-switch
                    v-model="securitySettings.accessControl.enabled"
          ...
          <el-card class="user-groups-card">
            <template #header>
              <div class="card-header">
                <h3>用户组管理</h3>
                <el-button type="primary" size="small" @click="addNewUserGroup">
              ...
              >
                <div class="group-header">
                  <div class="group-info">
                    <h4>{{ group.name }}</h4>
                    <span class="group-users">{{ group.userCount }} 个用户</span>
                  </div>
                  <div class="group-actions">
                    <el-button size="small" @click="editUserGroup(group)">
                      编辑
                    </el-button>
                    <el-button size="small" type="danger" @click="deleteUserGroup(group)">
                      删除
                    </el-button>
                  </div>
                </div>
                ...
                <div class="group-permissions">
                  ...
                  <div class="permission-tags">
                    ...
                    <el-tag v-if="group.permissions.length > 3" size="small" class="more-tag">
    ...
    <!-- 用户组编辑对话框 -->
    <el-dialog
      v-model="userGroupDialog.visible"
      :title="userGroupDialog.mode === 'add' ? '添加用户组' : '编辑用户组'"
      width="600px"
    >
      <el-form :model="userGroupForm" label-width="100px">
        <el-form-item label="组名称">
          <el-input v-model="userGroupForm.name" placeholder="请输入组名称" />
        </el-form-item>
        <el-form-item label="组描述">
          <el-input v-model="userGroupForm.description" type="textarea" placeholder="请输入组描述" />
        </el-form-item>
        <el-form-item label="权限设置">
          <div class="role-permissions">
      ...
      <template #footer>
        <el-button @click="userGroupDialog.visible = false">取消</el-button>
    ...
    <!-- 高级权限设置对话框 -->
    <el-dialog
      v-model="advancedPermissionDialog.visible"
      title="高级权限设置"
      width="800px"
    >
      <div class="advanced-permission-settings">
        <el-tabs v-model="activePermissionTab">
          <el-tab-pane label="权限继承" name="inheritance">
            <div class="inheritance-settings">
              ...
              <el-form :model="inheritanceForm" label-width="120px">
                <el-form-item label="启用继承">
                  <el-switch v-model="inheritanceForm.enabled" />
                </el-form-item>
                <el-form-item label="继承模式">
                  <el-radio-group v-model="inheritanceForm.mode">
                    <el-radio label="strict">严格继承</el-radio>
                    <el-radio label="relaxed">宽松继承</el-radio>
                    <el-radio label="custom">自定义继承</el-radio>
                  </el-radio-group>
                </el-form-item>
                <el-form-item label="继承规则">
                  <el-checkbox-group v-model="inheritanceForm.rules">
                    <el-checkbox label="inherit_permissions">继承权限</el-checkbox>
                    <el-checkbox label="inherit_roles">继承角色</el-checkbox>
                    <el-checkbox label="inherit_groups">继承用户组</el-checkbox>
                    <el-checkbox label="inherit_data_access">继承数据访问权限</el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
              </el-form>
            </div>
            ...
            <div class="delegation-settings">
              ...
              <el-form :model="delegationForm" label-width="120px">
                <el-form-item label="启用委托">
                  <el-switch v-model="delegationForm.enabled" />
                </el-form-item>
                <el-form-item label="委托级别">
                  <el-select v-model="delegationForm.level">
                    <el-option label="一级委托" value="level1" />
                    <el-option label="二级委托" value="level2" />
                    <el-option label="三级委托" value="level3" />
                  </el-select>
                </el-form-item>
                <el-form-item label="委托时间">
                  <el-date-picker
                    v-model="delegationForm.startTime"
                    type="datetime"
                    placeholder="开始时间"
                  />
                  <span>-</span>
                  <el-date-picker
                    v-model="delegationForm.endTime"
                    type="datetime"
                    placeholder="结束时间"
                  />
                </el-form-item>
                <el-form-item label="可委托权限">
                  <el-checkbox-group v-model="delegationForm.delegatablePermissions">
                    <el-checkbox label="user_manage">用户管理</el-checkbox>
                    <el-checkbox label="role_manage">角色管理</el-checkbox>
                    <el-checkbox label="content_manage">内容管理</el-checkbox>
          ...
          <el-tab-pane label="数据权限" name="data">
            <div class="data-permission-settings">
              <h4>数据权限配置</h4>
              <el-form :model="dataPermissionForm" label-width="120px">
                ...
                <el-form-item label="部门权限">
                  <el-checkbox-group v-model="dataPermissionForm.departments">
                    <el-checkbox label="tech">技术部</el-checkbox>
                    <el-checkbox label="content">内容部</el-checkbox>
                    <el-checkbox label="operation">运营部</el-checkbox>
                    <el-checkbox label="market">市场部</el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
            ...
            <div class="audit-settings">
              ...
              <el-form :model="auditForm" label-width="120px">
                <el-form-item label="启用审计">
                  <el-switch v-model="auditForm.enabled" />
                </el-form-item>
                <el-form-item label="审计级别">
                  <el-select v-model="auditForm.level">
                    <el-option label="基础审计" value="basic" />
                    <el-option label="详细审计" value="detailed" />
                    <el-option label="全面审计" value="comprehensive" />
                  </el-select>
                </el-form-item>
                <el-form-item label="审计事件">
                  <el-checkbox-group v-model="auditForm.events">
                    <el-checkbox label="login">登录事件</el-checkbox>
                    <el-checkbox label="permission_change">权限变更</el-checkbox>
                    <el-checkbox label="user_operation">用户操作</el-checkbox>
                    <el-checkbox label="data_access">数据访问</el-checkbox>
                    <el-checkbox label="system_operation">系统操作</el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
                <el-form-item label="保留时间">
                  <el-input-number
                    v-model="auditForm.retentionDays"
                    :min="30"
                    :max="365"
                    size="small"
                  />
      ...
      <template #footer>
        <el-button @click="advancedPermissionDialog.visible = false">取消</el-button>
        <el-button type="primary" @click="saveAdvancedPermissions">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import {
  ...
  View, Hide, Check, Close, Avatar, Star, Clock, Download,
  PieChart, TrendCharts, DataAnalysis, DataLine, DataBoard
...
const userGroups = ref([
  {
    id: 1,
    name: '技术团队',
    description: '技术开发和系统维护团队',
    userCount: 8,
    permissions: ['用户管理', '系统设置', '日志查看', '数据导出']
  },
  {
    id: 2,
    name: '内容团队',
    description: '内容创作和发布团队',
    userCount: 12,
    permissions: ['内容管理', '发布管理', '素材管理', '数据分析']
  },
  {
    id: 3,
    name: '运营团队',
    description: '市场运营和用户管理团队',
    userCount: 6,
    permissions: ['用户管理', '数据分析', '数据导出']
  }
])

const userGroupDialog = reactive({
  visible: false,
  mode: 'add'
})
...
// 高级权限设置对话框
const advancedPermissionDialog = reactive({
  visible: false
})

const activePermissionTab = ref('inheritance')

const inheritanceForm = reactive({
  enabled: false,
  mode: 'strict',
  rules: ['inherit_permissions', 'inherit_roles']
})
...
const delegationForm = reactive({
  ...
  startTime: '',
  endTime: '',
  delegatablePermissions: ['user_manage']
})
...
const auditForm = reactive({
  enabled: true,
  level: 'detailed',
  events: ['login', 'permission_change', 'user_operation'],
  retentionDays: 90
})
...
const togglePasswordPolicy = (enabled) => {
  ElMessage.success(`密码策略已${enabled ? '启用' : '禁用'}`)
}

const toggleLoginSecurity = (enabled) => {
  ElMessage.success(`登录安全已${enabled ? '启用' : '禁用'}`)
}

const toggleSessionManagement = (enabled) => {
  ElMessage.success(`会话管理已${enabled ? '启用' : '禁用'}`)
}

const toggleAccessControl = (enabled) => {
  ElMessage.success(`访问控制已${enabled ? '启用' : '禁用'}`)
}
...
const addNewUserGroup = () => {
  userGroupDialog.mode = 'add'
  Object.keys(userGroupForm).forEach(key => {
    userGroupForm[key] = key === 'permissions' ? [] : ''
  })
  userGroupDialog.visible = true
}

const editUserGroup = (group) => {
  userGroupDialog.mode = 'edit'
  Object.keys(userGroupForm).forEach(key => {
    userGroupForm[key] = group[key] || (key === 'permissions' ? [] : '')
  })
...
const saveUserGroup = () => {
  if (!userGroupForm.name || !userGroupForm.description) {
    ElMessage.warning('请填写必要信息')
    return
  }

  if (userGroupDialog.mode === 'add') {
    const newGroup = {
      id: userGroups.value.length + 1,
      ...userGroupForm,
      userCount: 0
    }
    ...
    const groupIndex = userGroups.value.findIndex(g => g.id === userGroupForm.id)
    if (groupIndex > -1) {
      userGroups.value[groupIndex] = { ...userGroups.value[groupIndex], ...userGroupForm }
      ElMessage.success('用户组更新成功')
    }
  }

  userGroupDialog.visible = false
}
...
const batchAssignRole = (role) => {
  ...
  ElMessage.success(`成功为 ${selectedUsers.value.length} 个用户分配 ${getRoleName(role)} 角色`)
  selectedUsers.value = []
}
...
// 权限继承和委托
const configurePermissionInheritance = () => {
  ElMessage.info('正在配置权限继承...')
}

const configurePermissionDelegation = () => {
  ElMessage.info('正在配置权限委托...')
}

// 数据权限管理
const configureDataPermissions = () => {
  ElMessage.info('正在配置数据权限...')
}

// 审计和监控
const viewSecurityAudit = () => {
  ElMessage.info('正在查看安全审计...')
}

const exportSecurityReport = () => {
  ElMessage.success('正在导出安全报告...')
}

// 高级权限配置
const configureAdvancedPermissions = () => {
  advancedPermissionDialog.visible = true
}

const saveAdvancedPermissions = () => {
  ElMessage.success('高级权限设置保存成功')
  advancedPermissionDialog.visible = false
}


// 添加高级权限管理按钮
const showAdvancedPermissionDialog = () => {
  advancedPermissionDialog.visible = true
}
